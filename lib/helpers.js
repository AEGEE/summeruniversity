...
exports.getDefaultQuery = async (req) => {
    ...
    // If season is set, filter on it.
    const seasonQuery = [];
    if (req.query.season_id) {
        const season = await Season.findOne({ where: { id: req.query.season_id } });
        seasonQuery.push({ starts: { [Sequelize.Op.gte]: moment(season.su_earliest_date, 'YYYY-MM-DD').startOf('day').toDate() } });
        seasonQuery.push({ ends: { [Sequelize.Op.lte]: moment(season.su_latest_date, 'YYYY-MM-DD').endOf('day').toDate() } });
    }
    queryObj.where[Sequelize.Op.and] = seasonQuery;
...
exports.getPermissions = (user, corePermissions, approvePermissions) => {
    ...
    permissions.list_seasons = hasPermission(corePermissions, 'manage_summeruniversity:regular');
    permissions.create_season = hasPermission(corePermissions, 'manage_summeruniversity:regular');
    permissions.view_season = hasPermission(corePermissions, 'manage_summeruniversity:regular');
    permissions.edit_season = hasPermission(corePermissions, 'manage_summeruniversity:regular');
