...
exports.getStats = async (req, res) => {
    ...
    for (let i = 0; i < statsObject.by_event.length; i++) {
        const eventName = await Event.findOne({ attributes: ['name'], where: { id: statsObject.by_event[i].type } });
        statsObject.by_event[i].type = eventName.name;
    }
