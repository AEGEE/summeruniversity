...
const { Application, Event, Season } = require('../models');
...
exports.fetchSingleSeason = async (req, res, next) => {
    const season = await Season.findOne({ where: req.params.season_id });

    if (!season) {
        return errors.makeNotFoundError(res, `Season with id ${req.params.season_id} not found`);
    }
