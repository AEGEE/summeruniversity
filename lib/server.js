...
const seasons = require('./seasons'); // API middlewares for seasons management
...
const SeasonsRouter = router({ mergeParams: true });
...
GeneralRouter.get('/seasons', middlewares.ensureAuthorized, seasons.listSeasons);
GeneralRouter.post('/seasons', middlewares.ensureAuthorized, seasons.addSeason);
...
EventsRouter.put('/applications/:application_id/cancelled', middlewares.fetchSingleApplication, applications.setApplicationCancelled);
...
SeasonsRouter.use(middlewares.ensureAuthorized, middlewares.fetchSingleSeason);
SeasonsRouter.get('/', seasons.getSeason);
SeasonsRouter.put('/', seasons.editSeason);

server.use(endpointsMetrics.addEndpointMetrics);
server.use('/', GeneralRouter);
server.use('/single/:event_id', EventsRouter);
server.use('/seasons/:season_id', SeasonsRouter);
